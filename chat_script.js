...
const showNextQuestion = () => {
  if (currentQuestionIndex < jsonData.questions.length) {
    ...
    const questionChatDiv =
      createChatElement(questionHtml, 'incoming');
...
const handleOptionClick = (e) => {
  const selectedOption = e.target.textContent.trim().toLowerCase();

  const selectedCategory =
    jsonData.questions[currentQuestionIndex].category;

  const dosha =
    jsonData.categories[selectedCategory][selectedOption];
  ...
  const selectedOptionChatDiv =
    createChatElement(selectedOptionHtml, 'outgoing');
